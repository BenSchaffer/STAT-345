---
title: 'Homework 11: Version Control and Workflow'
author: "STAT 345"
output:
...
- **1a.** Show me that you have a <tt>GitHub</tt> account. Create a repository on <tt>GitHub</tt> called "STAT 345". Then edit the code below so that I can see the contents of <tt>README.md</tt> for that repo. To get the correct URL, do the following: go to your <tt>GitHub</tt> repo, click on <tt>STAT 345</tt> and then again on <tt>README.md</tt>, and click on the "Raw" button. Copy and paste the URL to the raw <tt>README.md</tt> file into the call to <tt>readLines()</tt> below.

NOTE: If you encounter 404 errors make sure that your repo is public.

```{r}
readLines("https://raw.githubusercontent.com/BenSchaffer/STAT-345/master/README.md")
```

- **1b.** Show us that you have <tt>Git</tt> installed on your computer. Create a new project within **RStudio** that is tied to your "STAT 345" repo on <tt>GitHub</tt>. Then create a new <tt>R Script</tt> (and *not* an <tt>R Markdown</tt> file) in which you put <tt>print("Hello, world!")</tt>. Save this file (call it <tt>hello_world.R</tt>) to your local "STAT 345" repo. Stage the file, commit the file (and add a suitable commit message), and push the file to <tt>GitHub</tt>. Follow the steps above to find the URL to the raw file for <tt>hello_world.R</tt> and copy and paste that URL below in the call to <tt>source_url()</tt>. If everything works, "Hello, world!" should appear.
...
```{r}
# readLines("Enter the URL to relevant commit of hw11_simulation.R")
```
